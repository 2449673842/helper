...
import time   # <--- 用于时间戳 (如果需要，或者用datetime)
    ...
    if task_id_to_delete in download_records:
        ...
        if zip_filename_to_delete:
            ...
            if os.path.exists(zip_file_path_to_delete):
                try:
                    os.remove(
                        zip_file_path_to_delete); message += f" Associated ZIP file '{zip_filename_to_delete}' also deleted."
                except OSError as e:
...
def download_zip_package_route(filename):
    try:
        ...
        print(f"Error sending ZIP {filename}: {e}"); return jsonify({"error": "Server error sending ZIP."}), 500
